...
End If
If Cells(CurrRow, 3).Value = 0 And Cells(CurrRow, 4).Value = 1 Then
...
lastRowInS = Range("S" & Rows.Count).End(xlUp).Row
Range("R2").AutoFill Destination:=Range("R2:R" & lastRowInS)
End Sub
